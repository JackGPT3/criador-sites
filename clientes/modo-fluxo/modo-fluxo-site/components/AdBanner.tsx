...
export default function AdBanner({ slot = '', format = 'horizontal', className = '' }: AdBannerProps) {
  ...
  if (process.env.NODE_ENV === 'development') {
    return (
      <div className={`w-full ${sizes[format]} rounded-lg border border-dashed border-slate-700 bg-slate-800/30 flex items-center justify-center text-slate-500 text-xs gap-2 ${className}`}>
